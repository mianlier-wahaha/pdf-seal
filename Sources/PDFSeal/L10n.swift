import Foundation

/// 取本地化字符串（源语言为简体中文，键即中文原文）
func L(_ key: String) -> String { L10n.t(key) }

/// 带参数的本地化字符串（键中使用 %d / %@ 占位）
func LF(_ key: String, _ args: CVarArg...) -> String {
    String(format: L10n.t(key), locale: Locale.current, arguments: args)
}

enum L10n {
    /// 跟随系统语言：en / zh-Hans / zh-Hant（其他中文变体归入简体或繁体）
    static var langCode: String {
        let pref = Bundle.main.preferredLocalizations.first
            ?? Locale.preferredLanguages.first ?? "zh-Hans"
        if pref.hasPrefix("en") { return "en" }
        if pref.contains("Hant") || pref.contains("HK") || pref.contains("MO") { return "zh-Hant" }
        return "zh-Hans"
    }

    static func t(_ key: String) -> String {
        switch langCode {
        case "en": return en[key] ?? key
        case "zh-Hant": return zhHant[key] ?? key
        default: return key
        }
    }

    /// 另存为时追加到文件名末尾的标记，按界面语言取值。
    /// 注意：不能放进词典用 L() 取——简体分支直接返回 key，会把「文件名后缀」这类描述文字写进文件名。
    /// 取值原则：简短、无空格、语义通用（同时涵盖骑缝章与正文章，故不用「骑缝」）。
    static var stampedSuffix: String {
        switch langCode {
        case "en": return "stamped"
        case "zh-Hant": return "已蓋章"
        default: return "已盖章"
        }
    }

    static let en: [String: String] = [
        "PDF 骑缝章": "PDF Seal",
        "打开": "Open",
        "章体大小": "Stamp Size",
        "保存": "Save",
        "另存为": "Save As",
        "关闭": "Close",
        "该文件尚未保存，是否关闭？": "This file has unsaved changes. Close it?",
        "确认": "Confirm",
        "拼合校验": "Jigsaw Check",
        "出错了": "Error",
        "好": "OK",
        "取消": "Cancel",
        "把 PDF 拖进来，或点「打开」选择文件": "Drag a PDF here, or click “Open PDF…”",
        "打开 PDF…": "Open PDF…",
        "已载入 %d 页": "Loaded · %d pages",
        "已保存：%@": "Saved: %@",
        "已导出：%@": "Exported: %@",
        "请先打开 PDF": "Open a PDF first",
        "请先添加印章再保存": "Add a stamp before saving",
        "请先添加印章再导出": "Add a stamp before exporting",
        "请先添加印章或启用水印再保存": "Add a stamp or enable the watermark before saving",
        "请先添加印章或启用水印再导出": "Add a stamp or enable the watermark before exporting",
        "水印": "Watermark",
        "启用水印": "Enable Watermark",
        "水印文本": "Watermark text",
        "字体": "Font",
        "字号": "Size",
        "颜色": "Color",
        "旋转": "Rotation",
        "透明度": "Opacity",
        "多行水印": "Multi-row",
        "单行": "Single",
        "一页两行": "Two rows",
        "平铺": "Tiled",
        "水平位置": "Horizontal",
        "垂直位置": "Vertical",
        "水平偏移": "H-offset",
        "垂直偏移": "V-offset",
        "居左": "Left",
        "居中": "Center",
        "居右": "Right",
        "居上": "Top",
        "居下": "Bottom",
        "宋体-简": "Songti",
        "黑体": "Heiti",
        "楷体-简": "Kaiti",
        "仿宋": "Fangsong",
        "水印垫在章的下层；预览即导出效果": "Watermark sits beneath stamps; preview matches export",
        "骑缝章": "Seam Stamps",
        "缝位": "Edge",
        "右缝": "Right",
        "左缝": "Left",
        "全部页": "All Pages",
        "页码": "Page",
        "上": "Up",
        "下": "Down",
        "添加": "Add",
        "内页占比控制": "Control Middle Ratio",
        "首尾页各占首页占比，中间页均分剩余占比；尾页贴到印章最边缘": "First and last pages share the first-page ratio; middle pages split the remainder evenly. The last page reaches the seal's right edge.",
        "移除全部": "Remove All",
        "移除": "Remove",
        "移除这枚章（Delete 键）": "Remove this stamp (Delete)",
        "已选中第 %d 枚 / 共 %d 枚": "Stamp %d of %d selected",
        "正文章": "Body Stamps",
        "添加一枚按当前选项盖章的章；可多次添加多枚。添加后点击页面落位、拖动微调、右键章可删除":
            "Click Add to place a stamp with the current options — add as many as you need. After that: click to position, drag to fine-tune, right-click to delete.",
        "已添加 %d 枚章；点击预览中的章可选中它":
            "%d stamps added — click a stamp in the preview to select it",
        "已添加 %d 条骑缝章；调整本滑杆后点「添加」可错开上下位置":
            "%d seam stamps added — adjust the slider and click Add to stagger them vertically",
        "首页占比加大": "First Page Larger",
        "首页占比": "First Page Ratio",
        "内页占比": "Middle Pages Ratio",
        "开启后首页占上述比例，中间页各占内页占比，尾页取剩余并贴到最边缘": "When on, first page takes the ratio above; middle pages take the middle ratio; last page takes the remaining portion and reaches the edge",
        "首页占首页占比，中间页各占内页占比，尾页取剩余部分并贴到印章最边缘；各页占比之和超过 100% 时自动以多份印章平铺展示":
            "First page takes its ratio; middle pages take the middle ratio; last page takes the remainder and reaches the seal edge. When the sum exceeds 100%, the seal is tiled across multiple copies",
        "删除本页章": "Remove from This Page",
        "删除此章（所有页）": "Delete Stamp (All Pages)",
        "恢复本页章": "Restore on This Page",
        "删除本页骑缝章条": "Remove This Seam Slice",
        "删除此骑缝章（所有页）": "Delete Seam Stamp (All Pages)",
        "恢复本页骑缝章条": "Restore This Seam Slice",
        "缩放": "Zoom",
        "印章库": "Seal Library",
        "还没有印章\n点右上角 + 导入章图": "No seals yet\nClick + to import an image",
        "导入印章图片（PNG / JPG）": "Import seal image (PNG / JPG)",
        "选择印章图片（扫描或拍照的章图均可）": "Choose a seal image — scanned or photographed works",
        "删除": "Delete",
        "新建图章": "New Seal",
        "白色转成透明": "Make White Transparent",
        "容错": "Tolerance",
        "尺寸(cm)": "Size (cm)",
        "锁定比例": "Lock Aspect Ratio",
        "名称": "Name",
        "印章名称": "Seal name",
        "章预览": "Seal preview",
        "印章": "Seal",
        "保存印章失败": "Failed to save seal",
        "拖拽可排序；选中后按 ↑/↓ 上下移动": "Drag to reorder; ↑/↓ moves the selected seal",
        "保存印章失败：%@": "Failed to save seal: %@",
        "仅支持 PDF 文件": "Only PDF files are supported",
        "无法打开 PDF（可能已加密或损坏）": "Cannot open PDF (it may be encrypted or damaged)",
        "模拟拼合校验": "Seam Jigsaw Preview",
        "把每页边缘的章条按页序拼合，应能还原出右侧的完整印章":
            "The slices from page edges reassemble into the full seal on the right",
        "拼合结果": "Reassembled",
        "原始印章": "Original Seal",
        "请先选择印章": "Select a seal first",
        "当前页范围：第 %d — %d 页（共 %d 条）": "Page range: %d – %d (%d slices)",
    ]

    static let zhHant: [String: String] = [
        "PDF 骑缝章": "PDF 騎縫章",
        "打开": "打開",
        "章体大小": "章體大小",
        "保存": "儲存",
        "另存为": "另存為",
        "关闭": "關閉",
        "该文件尚未保存，是否关闭？": "該檔案尚未儲存，是否關閉？",
        "确认": "確認",
        "拼合校验": "拼合校驗",
        "出错了": "發生錯誤",
        "好": "好",
        "取消": "取消",
        "把 PDF 拖进来，或点「打开」选择文件": "將 PDF 拖進來，或點「打開」選擇文件",
        "打开 PDF…": "打開 PDF…",
        "已载入 %d 页": "已載入 %d 頁",
        "已保存：%@": "已儲存：%@",
        "已导出：%@": "已匯出：%@",
        "请先打开 PDF": "請先打開 PDF",
        "请先添加印章再保存": "請先新增印章再儲存",
        "请先添加印章再导出": "請先新增印章再匯出",
        "请先添加印章或启用水印再保存": "請先新增印章或啟用水印再儲存",
        "请先添加印章或启用水印再导出": "請先新增印章或啟用水印再匯出",
        "水印": "浮水印",
        "启用水印": "啟用浮水印",
        "水印文本": "浮水印文字",
        "字体": "字體",
        "字号": "字號",
        "颜色": "顏色",
        "旋转": "旋轉",
        "透明度": "透明度",
        "多行水印": "多行浮水印",
        "单行": "單行",
        "一页两行": "一頁兩行",
        "平铺": "平鋪",
        "水平位置": "水平位置",
        "垂直位置": "垂直位置",
        "水平偏移": "水平偏移",
        "垂直偏移": "垂直偏移",
        "居左": "靠左",
        "居中": "置中",
        "居右": "靠右",
        "居上": "靠上",
        "居下": "靠下",
        "宋体-简": "宋體",
        "黑体": "黑體",
        "楷体-简": "楷體",
        "仿宋": "仿宋",
        "水印垫在章的下层；预览即导出效果": "浮水印墊在章的下層；預覽即匯出效果",
        "文件名后缀": "騎縫章",
        "骑缝章": "騎縫章",
        "缝位": "縫位",
        "右缝": "右縫",
        "左缝": "左縫",
        "全部页": "全部頁面",
        "页码": "頁碼",
        "上": "上",
        "下": "下",
        "添加": "新增",
        "内页占比控制": "內頁佔比控制",
        "首尾页各占首页占比，中间页均分剩余占比；尾页贴到印章最边缘": "首尾頁各占首頁佔比，中間頁均分剩餘佔比；尾頁貼到印章最邊緣",
        "移除全部": "全部移除",
        "移除": "移除",
        "移除这枚章（Delete 键）": "移除此印章（Delete 鍵）",
        "已选中第 %d 枚 / 共 %d 枚": "已選取第 %d 枚 / 共 %d 枚",
        "正文章": "正文印章",
        "添加一枚按当前选项盖章的章；可多次添加多枚。添加后点击页面落位、拖动微调、右键章可删除":
            "每次「新增」會以目前選項放置一枚印章，可連續新增多枚。新增後點擊頁面落位、拖曳微調、右鍵章可刪除",
        "已添加 %d 枚章；点击预览中的章可选中它":
            "已新增 %d 枚印章；點擊預覽中的印章可選取它",
        "已添加 %d 条骑缝章；调整本滑杆后点「添加」可错开上下位置":
            "已新增 %d 條騎縫章；調整本滑桿後點「新增」可錯開上下位置",
        "首页占比加大": "首頁佔比加大",
        "首页占比": "首頁佔比",
        "内页占比": "內頁佔比",
        "开启后首页占上述比例，中间页各占内页占比，尾页取剩余并贴到最边缘": "開啟後首頁占上述比例，中間頁各占內頁佔比，尾頁取剩餘並貼到最邊緣",
        "首页占首页占比，中间页各占内页占比，尾页取剩余部分并贴到印章最边缘；各页占比之和超过 100% 时自动以多份印章平铺展示":
            "首頁占首頁佔比，中間頁各占內頁佔比，尾頁取剩餘部分並貼到印章最邊緣；各頁佔比合計超過 100% 時自動以多份印章平鋪呈現",
        "删除本页章": "刪除本頁印章",
        "删除此章（所有页）": "刪除此印章（所有頁面）",
        "恢复本页章": "回復本頁印章",
        "删除本页骑缝章条": "刪除本頁騎縫章條",
        "删除此骑缝章（所有页）": "刪除此騎縫章（所有頁面）",
        "恢复本页骑缝章条": "回復本頁騎縫章條",
        "缩放": "縮放",
        "印章库": "印章庫",
        "还没有印章\n点右上角 + 导入章图": "尚無印章\n點右上角 + 匯入章圖",
        "导入印章图片（PNG / JPG）": "匯入印章圖片（PNG / JPG）",
        "选择印章图片（扫描或拍照的章图均可）": "選擇印章圖片（掃描或拍照的章圖均可）",
        "删除": "刪除",
        "新建图章": "新增圖章",
        "白色转成透明": "白色轉為透明",
        "容错": "容錯",
        "尺寸(cm)": "尺寸 (cm)",
        "锁定比例": "鎖定比例",
        "名称": "名稱",
        "印章名称": "印章名稱",
        "章预览": "章預覽",
        "印章": "印章",
        "保存印章失败": "儲存印章失敗",
        "拖拽可排序；选中后按 ↑/↓ 上下移动": "拖曳可排序；選取後按 ↑/↓ 上下移動",
        "保存印章失败：%@": "儲存印章失敗：%@",
        "仅支持 PDF 文件": "僅支援 PDF 文件",
        "无法打开 PDF（可能已加密或损坏）": "無法打開 PDF（可能已加密或損壞）",
        "模拟拼合校验": "模擬拼合校驗",
        "把每页边缘的章条按页序拼合，应能还原出右侧的完整印章":
            "把每頁邊緣的章條按頁序拼合，應能還原出右側的完整印章",
        "拼合结果": "拼合結果",
        "原始印章": "原始印章",
        "请先选择印章": "請先選擇印章",
        "当前页范围：第 %d — %d 页（共 %d 条）": "目前頁面範圍：第 %d — %d 頁（共 %d 條）",
    ]
}
